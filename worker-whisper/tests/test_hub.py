...
def test_hub_presets_have_model_size(hub_data):
    """Each preset's env dict has a 'MODEL_SIZE' key."""
    for i, preset in enumerate(hub_data["presets"]):
        assert "MODEL_SIZE" in preset["env"], (
            f"Preset {i} ({preset['name']}) missing MODEL_SIZE in env"
        )
...
def test_hub_turbo_preset(hub_data):
    """A preset with MODEL_SIZE='large-v3-turbo' exists."""
    turbo_presets = [
        p for p in hub_data["presets"]
        if p["env"].get("MODEL_SIZE") == "large-v3-turbo"
    ]
    assert len(turbo_presets) > 0, "No preset with MODEL_SIZE='large-v3-turbo' found"


def test_hub_large_v3_preset(hub_data):
    """A preset with MODEL_SIZE='large-v3' exists."""
    large_v3_presets = [
        p for p in hub_data["presets"]
        if p["env"].get("MODEL_SIZE") == "large-v3"
    ]
    assert len(large_v3_presets) > 0, "No preset with MODEL_SIZE='large-v3' found"
